...
// whether this address triggers stack guard
bool trigger_stackguard(elf_core_info* core_info, Elf32_Addr address) {
	int segment = -1;
	int i = 0;
	...
	// address is in the last segment - assume it is stack region
	if (segment == core_info->phdr_num - 1) {
		Elf32_Addr pagesize = getpagesize();
		//LOG(stderr, "address %x pagesize %x, stack base %x\n", address,
		//	core_info->phdr[segment].p_vaddr, pagesize);
		if (address < core_info->phdr[segment].p_vaddr + pagesize) {
			LOG(stderr, "Stack Guard is Triggered\n");
		}
	} else {
		return false;
	}
...
int get_data_from_specified_file(elf_core_info *core_info, elf_binary_info *bin_info, Elf32_Addr address, char *buf, size_t buf_size){
	...
	// binary file, but not elf type
	if (target_file && !target_file->phdr) {
