...
static void add_dq_valset(valset_u *vt, valset_u *vt1, valset_u *vt2) {
	char *pv = &vt->byte;
	char *pv1 = &vt1->byte;
	char *pv2 = &vt2->byte;

	int i = 0;
	// go through valset and add byte by byte
...
// left rotate
static unsigned rol_short(unsigned short num, int bits){
      return ((num << bits) | (num >> (sizeof(short)*8-bits)));
}

//right rotate
static unsigned ror_short(unsigned short num, int bits){
      return ((num >> bits) | (num << (sizeof(short)*8-bits)));
}
...
void add_resolver(re_list_t* inst, re_list_t *re_deflist, re_list_t *re_uselist){
	...
	if(!CAST2_USE(src[0]->node)->val_known 
		...
		sign_extend_valset(&vs1, CAST2_USE(src[1]->node)->operand->datatype);
...
void sub_resolver(re_list_t* inst, re_list_t *re_deflist, re_list_t *re_uselist){
	...
	//print_info_of_current_inst(inst);

	instruction = re_ds.instlist + CAST2_INST(inst->node)->inst_index;
	if (strcmp(instruction->mnemonic, "sbb") == 0) {
		//FIXME: don't handle eflags now
		return;
	}
	...
	if(CAST2_USE(src[0]->node)->val_known 
		...
		if ((CAST2_USE(src[0]->node)->operand->datatype != op_byte) &&
		    (CAST2_USE(src[1]->node)->operand->type == op_immediate) &&
...
void mul_resolver(re_list_t* inst, re_list_t *re_deflist, re_list_t *re_uselist){
	...
	if (instruction->explicit_count != instruction->operand_count) {
		...
		if(CAST2_USE(src[0]->node)->val_known 
			...
			assert_val(dst[1], vt2, false);
			assert_val(dst[0], vt3, false);
		}
...
void mul_vs_handler(re_list_t *instnode) {
	...
	if (inst->explicit_count != inst->operand_count) {
		...
		if (!list_empty(&CAST2_USE(src[0]->node)->valset.list) &&
		    !list_empty(&CAST2_USE(src[1]->node)->valset.list)) {
			valset_mul_one(&CAST2_DEF(dst[0]->node)->aft_valset,
				&CAST2_DEF(dst[1]->node)->aft_valset,
				&CAST2_USE(src[0]->node)->valset,
				&CAST2_USE(src[1]->node)->valset
			);
		}
		...
		// eax = eax * ebx
		// imul eax, ebx, imm
		// eax = ebx * sign_extend(imm)
		assert(nuse == 2 && ndef == 1);
		if (!list_empty(&CAST2_USE(src[0]->node)->valset.list) &&
		    !list_empty(&CAST2_USE(src[1]->node)->valset.list)) {
			valset_mul_two_or_more(&CAST2_DEF(dst[0]->node)->aft_valset,
				&CAST2_USE(src[0]->node)->valset,
				&CAST2_USE(src[1]->node)->valset
			);
		}
...
void div_vs_handler(re_list_t *instnode) {
	...
	valset_div(&CAST2_DEF(dst[0]->node)->aft_valset,
		   &CAST2_DEF(dst[1]->node)->aft_valset,
		   &CAST2_USE(src[2]->node)->valset,
		   &CAST2_USE(src[1]->node)->valset,
		   &CAST2_USE(src[0]->node)->valset);
}
#endif
...
void dec_vs_handler(re_list_t *instnode) {
	...
	valset_sub_offset(&CAST2_DEF(dst[0]->node)->aft_valset,
		  &CAST2_USE(src[0]->node)->valset, 1);
}
#endif


void shld_handler(re_list_t * instnode){
...
void shl_handler(re_list_t * instnode){
	...
	// if it is shld instruction, forward to shld handler
	if (strcmp(inst->mnemonic, "shld") == 0) {
		shld_handler(instnode);
		return;
	}
	// for debugging use	
	print_all_operands(inst);
...
void shld_resolver(re_list_t* inst, re_list_t *re_deflist, re_list_t *re_uselist){

	re_list_t *entry;
	re_list_t *dst[NOPD], *src[NOPD];
	int nuse, ndef;
	valset_u vs1, vs2, vs3, vt;
	unsigned long long tmpll;
	unsigned long tmpl;

	traverse_inst_operand(inst, src, dst, re_uselist, re_deflist, &nuse, &ndef);	
	assert(nuse == 3 && ndef == 1);

	assert(CAST2_USE(src[2]->node)->operand->datatype == op_byte);
	...
	switch(CAST2_USE(src[0]->node)->operand->datatype){
		...
		case op_word:
			tmpl = vs1.word << 16 + vs2.word;
			vt.word = (tmpl << vs3.byte) >> 16;
			break;	
		case op_dword:
			tmpll = vs1.dword << 32 + vs2.dword;
			vt.dword = (tmpll << vs3.byte) >> 32;
			break;	
...
static bool process_other_shl_inst(re_list_t* instnode, re_list_t *re_deflist, re_list_t *re_uselist) {
	x86_insn_t *inst;
	
	inst= re_ds.instlist + CAST2_INST(instnode->node)->inst_index;

	if (strcmp(inst->mnemonic, "shl") == 0) {
		return true;
	}

	if (strcmp(inst->mnemonic, "shld") == 0) {
		shld_resolver(instnode, re_deflist, re_uselist);
		return false;
	}
}
...
void shl_resolver(re_list_t* inst, re_list_t *re_deflist, re_list_t *re_uselist){
	...
	if (!process_other_shl_inst(inst, re_deflist, re_uselist)) {
		return;
	}
...
void shl_vs_handler(re_list_t *instnode) {
	...
	// (including value set and address value) of operands
	vs_traverse_inst_operand(instnode, src, dst, &nuse, &ndef);
	// return if instruction is shld
	if (nuse == 3 && ndef == 1) return;
	...
	if(!CAST2_USE(src[0]->node)->val_known 
		...
		switch(CAST2_DEF(dst[0]->node)->operand->datatype){
			...
			case op_word:	
				switch (CAST2_USE(src[1]->node)->operand->datatype) {
					case op_byte:
						vt.word = ror_short(vs1.word, vs2.byte);
						break;
					default:
						assert(0);
				}
...
void rol_vs_handler(re_list_t *instnode) {
	...
	valset_rol(&CAST2_DEF(dst[0]->node)->aft_valset,
		   &CAST2_USE(src[0]->node)->valset,
		   &CAST2_USE(src[1]->node)->valset);
}
#endif
...
void ror_vs_handler(re_list_t *instnode) {
	...
	valset_ror(&CAST2_DEF(dst[0]->node)->aft_valset,
		   &CAST2_USE(src[0]->node)->valset,
		   &CAST2_USE(src[1]->node)->valset);
}
#endif
