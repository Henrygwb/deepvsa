...
static bool process_other_mov_inst(re_list_t *instnode, re_list_t *re_deflist, re_list_t *re_uselist) {
	...
	if (strcmp(inst->mnemonic, "movhpd") == 0) {
		movhpd_resolver(instnode, re_deflist, re_uselist);
		return false;
	}
...
void movsx_vs_handler(re_list_t *instnode){
	...
	sign_extend_value_set(&CAST2_DEF(dst[0]->node)->aft_valset,
		CAST2_DEF(dst[0]->node)->operand->datatype);
}
#endif


#if defined (ALIAS_MODULE) && (ALIAS_MODULE == VSA_MODULE)
static bool vs_other_mov_inst(re_list_t *instnode) {
	...
	if (strcmp(inst->mnemonic, "movsx") == 0) {
		movsx_vs_handler(instnode);
		return false;
	}

	if (strcmp(inst->mnemonic, "lea") == 0) {
		lea_vs_handler(instnode);
		return false;
	}
	...
	if (strcmp(inst->mnemonic, "movlpd") == 0) {
		movlpd_vs_handler(instnode);
		return false;
	}

	if (strcmp(inst->mnemonic, "movhpd") == 0) {
		movhpd_vs_handler(instnode);
		return false;
	}

	assert(0 && "go here if other mov instructions encounter");
}

void mov_vs_handler(re_list_t *instnode){

	if (!vs_other_mov_inst(instnode)) {
		return;
	}
...
void lea_vs_handler(re_list_t *instnode){
	...
	// Only Base or Index register, no corresponding operand

	vs_res_non_expression(dst[0]);

	// set dst[0] value set from Base and Index
	if (nuse == 0) {
		re_si si;
		unsigned long index = CAST2_INST(instnode->node)->inst_index;
		x86_op_t *opd = x86_get_src_operand(re_ds.instlist + index);
		INIT_SI(si, 0, opd->data.expression.disp, opd->data.expression.disp);
		add_new_valset(&CAST2_DEF(dst[0]->node)->aft_valset, CONST_REGION, 0, si);
	} else if (nuse == 1) {
		vs_res_reg_in_exp(src[0]);

		opd = CAST2_USE(src[0]->node)->operand;
		if (CAST2_USE(src[0]->node)->usetype == Base) {
			valset_add_offset(&CAST2_DEF(dst[0]->node)->aft_valset,
				&CAST2_USE(src[0]->node)->valset,
				opd->data.expression.disp);
		} else if (CAST2_USE(src[0]->node)->usetype == Index) {
		...
		// get constant from index and add it with disp
		// value add base and offset calculated above
		vs_res_reg_in_exp(src[0]);
		vs_res_reg_in_exp(src[1]);
		...
		valset_add(&CAST2_DEF(dst[0]->node)->aft_valset,
			   &CAST2_USE(src[0]->node)->valset, &tmp);
	}
}
#endif
...
void bswap_vs_handler(re_list_t *instnode){
	...
	head = &CAST2_USE(src[0]->node)->valset;

	if ((valset_is_empty(head)) ||
	    (!valset_is_empty(&CAST2_DEF(dst[0]->node)->aft_valset))) {
		return;
	}

	// Cannot get any bound when not const region
	if (!valset_is_const(head)) return;
	
	tmp_si = &(list_first_entry(&head->list, re_value_set, list)->si);
	
	if (tmp_si->lower_bound != tmp_si->upper_bound) {
		si_set_full(&si);
	} else {
		INIT_SI(si, 0,
			REVERSE_UINT(tmp_si->lower_bound),
			REVERSE_UINT(tmp_si->lower_bound));
	}
...
#if defined (ALIAS_MODULE) && (ALIAS_MODULE == VSA_MODULE)
static bool vs_process_other_xchg_inst(re_list_t *instnode) {
	x86_insn_t *inst;
...
void xchg_vs_handler(re_list_t *instnode){
	if (!vs_process_other_xchg_inst(instnode)) {
		return;
	}
	...
	assign_def_after_value_set(dst[0], &CAST2_USE(src[1]->node)->valset);
	assign_def_after_value_set(dst[1], &CAST2_USE(src[0]->node)->valset);
}
#endif

static void fix_bug_in_disasm(re_list_t *instnode) {
	...
	assert(dstopd->datatype == srcopd->datatype);

	if (srcopd->datatype != eax->datatype) {
		LOG(stdout, "Fix bug in the libdisasm here\n");
		switch (srcopd->datatype) {
		case op_dword:
			eax->datatype = srcopd->datatype;
			x86_reg_from_id(get_eax_id(), &eax->data.reg);
			break;
		default:
			assert(0);
		}	
...
void xchgcc_handler(re_list_t *instnode){
	...
	assert(strcmp(inst->mnemonic, "cmpxchg") == 0);
	...
	// fix one bug from libdisasm
	fix_bug_in_disasm(instnode);
...
void xchgcc_vs_handler(re_list_t *instnode){
	LOG(stdout, "FIXME: xchgcc vs handler\n");
	#warning no implementation for instruction xchgcc
	// accumulator = AL, AX, or EAX, depending on whether
	// a byte, word, or doubleword comparison is being performed
	// verify if accumulator == Destination
