...
void ptest_vs_handler(re_list_t* instnode){
	test_vs_handler(instnode);
}
#endif


void psubb_handler(re_list_t* instnode){
	sub_handler(instnode);
}


void psubb_resolver(re_list_t* inst, re_list_t* deflist, re_list_t* uselist){
	// do not reuse sub_resolver here
	sub_resolver(inst, deflist, uselist);
}
...
void por_resolver(re_list_t* inst, re_list_t* deflist, re_list_t* uselist){
	or_resolver(inst, deflist, uselist);
}
...
void pand_handler(re_list_t* instnode){
	and_handler(instnode);
}


void pand_resolver(re_list_t* inst, re_list_t* deflist, re_list_t* uselist){
	and_resolver(inst, deflist, uselist);
}
...
void pcmpgtb_resolver(re_list_t * inst, re_list_t* deflist, re_list_t * uselist){
...
void movhpd_resolver(re_list_t* inst, re_list_t *re_deflist, re_list_t *re_uselist){
	...
	if(CAST2_USE(src[0]->node)->val_known 
		...
		if (isdstxmm) {
			vt = CAST2_DEF(dst[0]->node)->afterval;
			// [0..63] <= [64..128]
			vt.dqword[0] = vt.dqword[2];
			vt.dqword[1] = vt.dqword[3];
...
/*
	...
	mdst = _mm_alignr_epi8(msr1, msr2, (int)CAST2_USE(src[2]->node)->val.byte);
...
void pmaxub_resolver(re_list_t *inst, re_list_t *deflist, re_list_t *uselist) {
