...
void push_vs_handler(re_list_t *instnode) {
	...
	fork_value_set(&head, &CAST2_USE(src[0]->node)->valset);

	if (CAST2_USE(src[0]->node)->operand->type == op_immediate) {
		sign_extend_value_set(&head, CAST2_USE(src[0]->node)->operand->datatype);
	}

	assign_def_after_value_set(dst[0], &head);
...
void pop_vs_handler(re_list_t *instnode) {
	...
	valset_add_offset(&CAST2_DEF(dst[0]->node)->aft_valset,
		   &CAST2_DEF(dst[0]->node)->bef_valset,
		   ADDR_SIZE_IN_BYTE);
...
void leave_vs_handler(re_list_t *instnode) {
	...
	assign_def_after_value_set(dst[2], &CAST2_USE(src[1]->node)->valset);
