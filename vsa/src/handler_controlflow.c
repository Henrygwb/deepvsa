...
static void verify_relation_for_jmp(re_list_t *instnode) {
	// jmp eax / call [eax]
	// eax / [eax] = the address of previnst
}
...
void jmp_vs_handler(re_list_t *instnode){
	...
	previnst = re_ds.instlist+CAST2_INST(instnode->node)->inst_index-1;
...
static void verify_relation_for_call(re_list_t *instnode) {
	// call eax / call [eax]
	// eax / [eax] = the address of previnst
}
...
void call_vs_handler(re_list_t *instnode){
	...
	//vt.dword -= ADDR_SIZE_IN_BYTE;
	//assign_value_use_value
	valset_add_offset(&CAST2_DEF(dst[1]->node)->aft_valset,
		   &CAST2_DEF(dst[1]->node)->bef_valset,
		   -ADDR_SIZE_IN_BYTE);
	...
	//verify_relation_for_call(instnode);
}
#endif
...
static void verify_relation_for_return(re_list_t *instnode){
	// [esp] = address of previnst has been used
}
...
void return_vs_handler(re_list_t *instnode){
	...
	if (re_ds.instlist[inst_index].explicit_count == 1) {
		assert(nuse == 2 && ndef == 2);
		assert(CAST2_USE(src[0]->node)->operand->datatype == op_word);

		disp += CAST2_USE(src[0]->node)->val.word;
		assign_use_value_set(src[1], &CAST2_DEF(dst[1]->node)->aft_valset);
	} else {
		assert(nuse == 1 && ndef == 2);
		assign_use_value_set(src[0], &CAST2_DEF(dst[1]->node)->aft_valset);
	}

	valset_add_offset(&CAST2_DEF(dst[0]->node)->aft_valset,
		   &CAST2_DEF(dst[0]->node)->bef_valset, disp);

	// post heuristic with control flow
	//verify_relation_for_return(instnode);
}
#endif
