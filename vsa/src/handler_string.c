...
void strload_handler(re_list_t *instnode){
	...
	//rint_info_of_current_inst(instnode);
}
...
void strsca_resolver(re_list_t* inst, re_list_t *re_deflist, re_list_t *re_uselist){
	...
	// if ecx is unknown or ecx == 0, directly return
	if (!CAST2_USE(src[0]->node)->val_known ||
	    ((CAST2_USE(src[0]->node)->val_known) && 
	     (CAST2_USE(src[0]->node)->val.dword == 0))) {
		// directly return
		return;
	}
...
static void verify_relation_for_strsca(re_list_t *instnode){
}

#if defined (ALIAS_MODULE) && (ALIAS_MODULE == VSA_MODULE)
void strsca_vs_handler(re_list_t *instnode){
	...
	df = resolve_df(instnode);

	size = translate_datatype_to_byte(x86_get_dest_operand(inst)->datatype);
	...
	// edi+=1/2/4 | edi-=1/2/4
	offset = (df) ? (-size) : (size);
	valset_add_offset(&CAST2_DEF(dst[1]->node)->aft_valset,
			  &CAST2_USE(src[1]->node)->valset,
			  offset);

	// post heuristic with control flow
	//verify_relation_for_strsca(instnode);
}
#endif
...
static void verify_relation_for_strcmp(re_list_t *instnode) {
}
...
void strcmp_vs_handler(re_list_t *instnode){
	...
	valset_add_offset(&CAST2_DEF(dst[2]->node)->aft_valset,
			  &CAST2_USE(src[2]->node)->valset,
			  offset);

	// post heuristic with control flow
	//verify_relation_for_strcmp(instnode);
}
#endif
...
void movs_vs_handler(re_list_t *instnode){
	...
	// [edi] = [esi]
	// dst[2] = src[2]
	assign_def_after_value_set(dst[2], &CAST2_USE(src[2]->node)->valset);
}
#endif
...
void strmov_resolver(re_list_t* inst, re_list_t *re_deflist, re_list_t *re_uselist){
	...
	size = translate_datatype_to_byte(CAST2_USE(src[3]->node)->operand->datatype);
...
void strmov_vs_handler(re_list_t *instnode){
	...
	// [edi] = [esi]
	// dst[3] = src[3]
	assign_def_after_value_set(dst[3], &CAST2_USE(src[3]->node)->valset);
}
#endif
...
void stos_vs_handler(re_list_t *instnode){
	...
	// [edi] = al/ax/eax
	// dst[2] = src[2]
	assign_def_after_value_set(dst[1], &CAST2_USE(src[1]->node)->valset);
}
#endif
