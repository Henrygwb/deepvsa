	...
	if ((reg.type&reg_gen)||(reg.type&reg_sp)||(reg.type&reg_fp)||(reg.type&reg_pc)) {
		...
		case 1:
			   ...
			   (strcmp(reg.name, "dh") == 0) ) {
            			value->byte = (gen_value & 0x0000ff00) >> 8;
			} else {
...
int get_memval_from_coredump(re_list_t *entry, valset_u *value) {
	...
	if (p == NULL) {
		LOG(stderr, "LOG: Error address is 0x%x\n", address);
		return BAD_ADDRESS;
	} 
...
x86_op_t * x86_implicit_operand_4th( x86_insn_t *inst ) {
	x86_oplist_t *temp;
	int count = 0;
	for(temp = inst->operands;temp; temp = temp->next, count++) {
		if (count == inst->explicit_count) {
...
void check_offset_type() {
	...
	list_for_each_entry(entry, &re_ds.head.list, list) {
		...
		if (opd->type == op_offset) {
			print_operand(*opd);
			assert(0 && "Modify handlers to remove op_offset");
		}
	}
}
