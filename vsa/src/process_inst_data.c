	...
	// load xmm log
	{
		...
		if(file = fopen(get_xmm_path(), "r")){
			if(fread(coredata->corereg.xmm_reg, sizeof(long), 32, file) == 0) {
				LOG(stderr, "ERROR: file read errori\n");
			}

			LOG(stdout, "After Loading XMM from outside log file\n");
			int i = 0;
			for (i=0; i<32; i++) {
				if (i % 4 == 0) LOG(stdout, "XMM[%d]:\t", i/4);
				LOG(stdout, "%08lx ", coredata->corereg.xmm_reg[i]);
				if (i % 4 == 3) LOG(stdout, "\n");
			}
		}	
	}
...
unsigned long load_trace(elf_core_info* core_info, elf_binary_info * binary_info, char *trace_file, x86_insn_t *instlist){
	...
	//LOG(stdout, "The address of the current instruction is %x\n", address);	
