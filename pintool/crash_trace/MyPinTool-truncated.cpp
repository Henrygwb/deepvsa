...
UINT64 totalIns = 0;	    // estimated number of dynamically executed instructions

UINT64 insCount = 0;        // number of dynamically executed instructions

#define NUM_TO_RECORD 1500000
...
VOID Get_Total_InsNum(){
    FILE * tins = fopen("inscount.out", "r");

    fscanf(tins, "%llu\n", &totalIns);

    printf("Total Instruction Number %llu\n", totalIns);
}
...
{
	if (insCount < totalIns - NUM_TO_RECORD) {
		return;
    	}
